(function () {
	...
	function getExtraHeight(m) {
		var totalExtraHeight = m.margins.top + m.margins.bottom
	...
	function getExtraWidth(m) {
		var totalExtraWidth = m.margins.left + m.margins.right
	...
	var DEFAULT_CHART_COLORS = ['light-blue', 'blue', 'violet', 'red', 'orange',
	...
	class SvgTip {
		...
		fill() {
			...
			this.listValues.map(function (set, i) {
				var color = this$1.colors[i] || 'black';
				...
				this$1.dataPointList.appendChild(li);
			});
		}
		...
		setValues(x, y, title, listValues, index) {
	 ...
	 */
	function fillArray(array, count, element, start) {
	...
	function equilizeNoOfElements(array1, array2,
		extraCount) {
		if ( extraCount === void 0 ) extraCount = array2.length - array1.length;
	...
	function getSplineCurvePointsStr(xList, yList) {
		...
		var pointStr = function (points, command) {
			return points.reduce(function (acc, point, i, a) { return i === 0
				? ((point[0]) + "," + (point[1]))
	...
	function makeSVGGroup(className, transform, parent) {
	...
	function makePath(pathStr, className, stroke, fill, strokeWidth) {
	...
	function makeArcPathStr(startPosition, endPosition, center, radius, clockWise, largeArc){
		if ( clockWise === void 0 ) clockWise=1;
		if ( largeArc === void 0 ) largeArc=0;

		var ref = [center.x + startPosition.x, center.y + startPosition.y];
		var arcStartX = ref[0];
		var arcStartY = ref[1];
		var ref$1 = [center.x + endPosition.x, center.y + endPosition.y];
		var arcEndX = ref$1[0];
		var arcEndY = ref$1[1];
		return ("M" + (center.x) + " " + (center.y) + "\n\t\tL" + arcStartX + " " + arcStartY + "\n\t\tA " + radius + " " + radius + " 0 " + largeArc + " " + (clockWise ? 1 : 0) + "\n\t\t" + arcEndX + " " + arcEndY + " z");
	}

	function makeCircleStr(startPosition, endPosition, center, radius, clockWise, largeArc){
		if ( clockWise === void 0 ) clockWise=1;
		if ( largeArc === void 0 ) largeArc=0;
		...
		var arcStartX = ref[0];
		var arcStartY = ref[1];
		var ref$1 = [center.x + endPosition.x, center.y * 2, center.y + endPosition.y];
		var arcEndX = ref$1[0];
		var midArc = ref$1[1];
		var arcEndY = ref$1[2];
		return ("M" + (center.x) + " " + (center.y) + "\n\t\tL" + arcStartX + " " + arcStartY + "\n\t\tA " + radius + " " + radius + " 0 " + largeArc + " " + (clockWise ? 1 : 0) + "\n\t\t" + arcEndX + " " + midArc + " z\n\t\tL" + arcStartX + " " + midArc + "\n\t\tA " + radius + " " + radius + " 0 " + largeArc + " " + (clockWise ? 1 : 0) + "\n\t\t" + arcEndX + " " + arcEndY + " z");
	...
	function makeArcStrokePathStr(startPosition, endPosition, center, radius, clockWise, largeArc){
		if ( clockWise === void 0 ) clockWise=1;
		if ( largeArc === void 0 ) largeArc=0;
	...
	function makeStrokeCircleStr(startPosition, endPosition, center, radius, clockWise, largeArc){
		if ( clockWise === void 0 ) clockWise=1;
		if ( largeArc === void 0 ) largeArc=0;
		...
		var arcStartX = ref[0];
		var arcStartY = ref[1];
		var ref$1 = [center.x + endPosition.x, radius * 2 + arcStartY, center.y + startPosition.y];
		var arcEndX = ref$1[0];
		var midArc = ref$1[1];
		var arcEndY = ref$1[2];
	...
	function makeGradient(svgDefElem, color, lighter) {
	...
	function percentageBar(x, y, width, height,
		depth, fill) {
		if ( depth === void 0 ) depth=PERCENTAGE_BAR_DEFAULT_DEPTH;
		if ( fill === void 0 ) fill='none';
	...
	function heatSquare(className, x, y, size, fill, data) {
		if ( fill === void 0 ) fill='none';
		if ( data === void 0 ) data={};
	...
	function legendBar(x, y, size, fill, label, truncate) {
		if ( fill === void 0 ) fill='none';
	...
	function legendDot(x, y, size, fill, label, truncate) {
		if ( fill === void 0 ) fill='none';
		if ( truncate === void 0 ) truncate=false;
	...
	function makeHoriLine(y, label, x1, x2, options) {
		...
		var className = 'line-horizontal ' + options.className +
	...
	function yMarker(y, label, width, options) {
		...
		var x = options.labelPos === 'left' ? LABEL_MARGIN
	...
	function datasetBar(x, yTop, width, color, label, index, offset, meta) {
		if ( label === void 0 ) label='';
		if ( index === void 0 ) index=0;
		if ( offset === void 0 ) offset=0;
		if ( meta === void 0 ) meta={};

		var ref = getBarHeightAndYAttr(yTop, meta.zeroLine);
		var height = ref[0];
		var y = ref[1];
		y -= offset;
	...
	function datasetDot(x, y, radius, color, label, index) {
		if ( label === void 0 ) label='';
		if ( index === void 0 ) index=0;
	...
	function getPaths(xList, yList, color, options, meta) {
		...
		var pointsList = yList.map(function (y, i) { return (xList[i] + ',' + y); });
	...
	var updateOverlay = {
		'bar': function (unit, overlay) {
			...
			Object.values(unit.attributes)
				.filter(function (attr) { return attributes.includes(attr.name) && attr.specified; })
				.map(function (attr) {
		...
		'heat_square': function (unit, overlay) {
			var transformValue;
			if(unit.nodeName !== 'circle') {
				transformValue = unit.getAttribute('transform');
				unit = unit.childNodes[0];
	...
	function animateBar(bar, x, yTop, width, offset, meta) {
		if ( offset === void 0 ) offset=0;
		if ( meta === void 0 ) meta={};
	...
	function animatePath(paths, newXList, newYList, zeroLine, spline) {
		var pathComponents = [];
		var pointsStr = newYList.map(function (y, i) { return (newXList[i] + ',' + y); }).join("L");
	...
	function animateSVGElement(element, props, dur, easingType, type, oldValues) {
		if ( easingType === void 0 ) easingType="linear";
	...
	function animateSVG(svgContainer, elements) {
		...
		elements.map(function (element) {
			...
			element[0] = unit;
			(assign = animateSVGElement.apply(void 0, element), animElement = assign[0], newElement = assign[1]);
	...
	class BaseChart {
		...
		configure() {
			...
			// Bind window events
			this.boundDrawFn = function () { return this$1.draw(true); };
		...
		draw(onlyWidthChange, init) {
			var this$1 = this;
			if ( onlyWidthChange === void 0 ) onlyWidthChange=false;
			...
			this.components.forEach(function (c) { return c.setup(this$1.drawArea); });
			// this.components.forEach(c => c.make());
			this.render(this.components, false);

			if(init) {
				this.data = this.realData;
				setTimeout(function () {this$1.update(this$1.data);}, this.initTimeout);
			}
		...
		render(components, animate) {
			...
			if(this.config.isNavigable) {
				...
				this.overlays.map(function (o) { return o.parentNode.removeChild(o); });
				// ref.parentNode.insertBefore(element, ref);
			}
			...
			if(elementsToAnimate.length > 0) {
				...
				setTimeout(function () {
					components.forEach(function (c) { return c.make(); });
					this$1.updateNav();
		...
		setupNavigation(init) {
			...
			if(init) {
				...
				document.addEventListener('keydown', function (e) {
					if(isElementInViewport(this$1.container)) {
						e = e || window.event;
						if(this$1.keyActions[e.keyCode]) {
							this$1.keyActions[e.keyCode]();
						}
					}
				});
			}
	...
	class AggregationChart extends BaseChart {
		...
		calc() {
			...
			var allTotals = this.data.labels.map(function (label, i) {
				var total = 0;
				this$1.data.datasets.map(function (e) {
					total += e.values[i];
				});
				return [total, label];
			}).filter(function (d) { return d[0] >= 0; }); // keep only positive results

			var totals = allTotals;
			if(allTotals.length > maxSlices) {
				// Prune and keep a grey area for rest as per maxSlices
				allTotals.sort(function (a, b) { return b[0] - a[0]; });

				totals = allTotals.slice(0, maxSlices-1);
				var remaining = allTotals.slice(maxSlices-1);

				var sumOfRemaining = 0;
				remaining.map(function (d) {sumOfRemaining += d[0];});
			...
			s.grandTotal = s.sliceTotals.reduce(function (a, b) { return a + b; }, 0);
		...
		renderLegend() {
			...
			this.legendTotals.map(function (d, i) {
				var barWidth = 110;
				var divisor = Math.floor(
					(this$1.width - getExtraWidth(this$1.measures))/barWidth
				);
				if (this$1.legendTotals.length < divisor) {
					barWidth = this$1.width/this$1.legendTotals.length;
				}
				if(count > divisor) {
					count = 0;
					y += 20;
				...
				var dot = legendDot(
					...
					this$1.colors[i],
					((s.labels[i]) + ": " + d),
					this$1.config.truncateLegends
				);
				this$1.legendArea.appendChild(dot);
				count++;
			});
		}
	}
	...
	class ChartComponent {
		constructor(ref) {
		var layerClass = ref.layerClass; if ( layerClass === void 0 ) layerClass = '';
		var layerTransform = ref.layerTransform; if ( layerTransform === void 0 ) layerTransform = '';
		var constants = ref.constants;
		var getData = ref.getData;
		var makeElements = ref.makeElements;
		var animateElements = ref.animateElements;
		...
		render(data) {
			...
			this.store.forEach(function (element) {
				this$1.layer.appendChild(element);
			});
	...
	var componentConfigs = {
		donutSlices: {
			...
			animateElements: function animateElements(newData) {
				return this.store.map(function (slice, i) { return animatePathStr(slice, newData.sliceStrings[i]); });
			},
		},
		pieSlices: {
		...
		percentageBars: {
			...
			makeElements: function makeElements(data) {
				...
				return data.xPositions.map(function (x, i) {
					var y = 0;
					var bar = percentageBar(x, y, data.widths[i],
						this$1.constants.barHeight, this$1.constants.barDepth, data.colors[i]);
					return bar;
				});
			},
		...
		yAxis: {
			...
			makeElements: function makeElements(data) {
				...
				return data.positions.map(function (position, i) { return yLine(position, data.labels[i], this$1.constants.width,
						{mode: this$1.constants.mode, pos: this$1.constants.pos, shortenNumbers: this$1.constants.shortenNumbers}); }
				);
			},
			...
			animateElements: function animateElements(newData) {
				...
				(assign = equilizeNoOfElements(oldPos, newPos), oldPos = assign[0], newPos = assign[1]);
				(assign$1 = equilizeNoOfElements(oldLabels, newLabels), oldLabels = assign$1[0], newLabels = assign$1[1]);
		...
		xAxis: {
			...
			makeElements: function makeElements(data) {
				...
				return data.positions.map(function (position, i) { return xLine(position, data.calcLabels[i], this$1.constants.height,
						{mode: this$1.constants.mode, pos: this$1.constants.pos}); }
				);
			},
		...
		yMarkers: {
			...
			makeElements: function makeElements(data) {
				...
				return data.map(function (m) { return yMarker(m.position, m.label, this$1.constants.width,
			...
			animateElements: function animateElements(newData) {
				...
				(assign = equilizeNoOfElements(this.oldData, newData), this.oldData = assign[0], newData = assign[1]);

				var newPos = newData.map(function (d) { return d.position; });
				var newLabels = newData.map(function (d) { return d.label; });
				var newOptions = newData.map(function (d) { return d.options; });

				var oldPos = this.oldData.map(function (d) { return d.position; });
		...
		yRegions: {
			...
			makeElements: function makeElements(data) {
				...
				return data.map(function (r) { return yRegion(r.startPos, r.endPos, this$1.constants.width,
			...
			animateElements: function animateElements(newData) {
				...
				var newPos = newData.map(function (d) { return d.endPos; });
				var newLabels = newData.map(function (d) { return d.label; });
				var newStarts = newData.map(function (d) { return d.startPos; });
				var newOptions = newData.map(function (d) { return d.options; });

				var oldPos = this.oldData.map(function (d) { return d.endPos; });
				var oldStarts = this.oldData.map(function (d) { return d.startPos; });
		...
		heatDomain: {
			...
			makeElements: function makeElements(data) {
				...
				var ref = this.constants;
				var index = ref.index;
				var colWidth = ref.colWidth;
				var rowHeight = ref.rowHeight;
				var squareSize = ref.squareSize;
				var xTranslate = ref.xTranslate;
				var monthNameHeight = -12;
				var x = xTranslate, y = 0;
				...
				data.cols.map(function (week, weekNo) {
					...
					week.map(function (day, i) {
						if(day.fill) {
							...
							var square = heatSquare('day', x, y, squareSize, day.fill, data);
							this$1.serializedSubDomains.push(square);
						}
						y += rowHeight;
					});
					y = 0;
		...
		barGraph: {
			...
			animateElements: function animateElements(newData) {
				...
				(assign = equilizeNoOfElements(oldXPos, newXPos), oldXPos = assign[0], newXPos = assign[1]);
				(assign$1 = equilizeNoOfElements(oldYPos, newYPos), oldYPos = assign$1[0], newYPos = assign$1[1]);
				(assign$2 = equilizeNoOfElements(oldOffsets, newOffsets), oldOffsets = assign$2[0], newOffsets = assign$2[1]);
				(assign$3 = equilizeNoOfElements(oldLabels, newLabels), oldLabels = assign$3[0], newLabels = assign$3[1]);
		...
		lineGraph: {
			...
			animateElements: function animateElements(newData) {
				...
				(assign = equilizeNoOfElements(oldXPos, newXPos), oldXPos = assign[0], newXPos = assign[1]);
				(assign$1 = equilizeNoOfElements(oldYPos, newYPos), oldYPos = assign$1[0], newYPos = assign$1[1]);
				(assign$2 = equilizeNoOfElements(oldValues, newValues), oldValues = assign$2[0], newValues = assign$2[1]);
	...
	function getComponent(name, constants, getData) {
		var keys = Object.keys(componentConfigs).filter(function (k) { return name.includes(k); });
	...
	class PercentageChart extends AggregationChart {
		...
		setupComponents() {
			...
			this.components = new Map(componentConfigs
				.map(function (args) {
					var component = getComponent.apply(void 0, args);
		...
		calc() {
			...
			s.sliceTotals.map(function (value) {
				var width = this$1.width * value / s.grandTotal;
				s.widths.push(width);
		...
		bindTooltip() {
			...
			this.container.addEventListener('mousemove', function (e) {
				var bars = this$1.components.get('percentageBars').store;
				var bar = e.target;
				if(bars.includes(bar)) {

					var i = bars.indexOf(bar);
					var gOff = getOffset(this$1.container), pOff = getOffset(bar);

					var x = pOff.left - gOff.left + parseInt(bar.getAttribute('width'))/2;
					var y = pOff.top - gOff.top;
					var title = (this$1.formattedLabels && this$1.formattedLabels.length>0
						? this$1.formattedLabels[i] : this$1.state.labels[i]) + ': ';
					var fraction = s.sliceTotals[i]/s.grandTotal;

					this$1.tip.setValues(x, y, {name: title, value: (fraction*100).toFixed(1) + "%"});
					this$1.tip.showTip();
				}
			});
		}
	...
	class PieChart extends AggregationChart {
		...
		calc() {
			...
			s.sliceTotals.map(function (total, i) {
				...
				var prevProperty = this$1.init && prevSlicesProperties[i];
				...
				var curPath =
					originDiffAngle === 360
						? makeCircleStr(curStart, curEnd, this$1.center, this$1.radius, clockWise, largeArc)
						: makeArcPathStr(curStart, curEnd, this$1.center, this$1.radius, clockWise, largeArc);
		...
		hoverSlice(path,i,flag,e){
			...
			if(flag) {
				...
				var title = (this.formatted_labels && this.formatted_labels.length > 0
	...
	function getChartRangeIntervals(max, min) {
		if ( min === void 0 ) min=0;
	...
	function getChartIntervals(maxValue, minValue) {
		...
		var ref = normalize(maxValue);
		var normalMaxValue = ref[0];
		var exponent = ref[1];
		...
		intervals = intervals.map(function (value) { return value * Math.pow(10, exponent); });
		return intervals;
	}

	function calcChartIntervals(values, withMinimum) {
		if ( withMinimum === void 0 ) withMinimum=false;
		...
		var exponent = 0, intervals = []; // eslint-disable-line no-unused-vars
		...
		// CASE II: Only minValue negative
			...
			if(maxValue >= absMinValue) {
				...
				intervals = posIntervals.map(function (d) { return d * (-1); });
			}
		...
		// CASE III: Both non-positive
			...
			intervals = intervals.reverse().map(function (d) { return d * (-1); });
		}
	...
	function getClosestInArray(goal, arr, index) {
	...
	function calcDistribution(values, distributionSize) {
		...
		var dataMaxValue = Math.max.apply(Math, values);
	...
	function getMaxCheckpoint(value, distribution) {
		return distribution.filter(function (d) { return d < value; }).length;
	}
	...
	class Heatmap extends BaseChart {
		constructor(parent, options) {
			...
			var startSubDomain = validStarts.includes(options.startSubDomain)
		...
		setupComponents() {
			...
			var componentConfigs = s.domainConfigs.map(function (config, i) { return [
				'heatDomain',
				{
					index: config.index,
					colWidth: COL_WIDTH,
					...
					xTranslate: s.domainConfigs
						.filter(function (config, j) { return j < i; })
						.map(function (config) { return config.cols.length - lessCol; })
			...
			DAY_NAMES_SHORT.forEach(function (dayName, i) {
				if([1, 3, 5].includes(i)) {
					...
					this$1.drawArea.appendChild(dayText);
				}
				y += ROW_HEIGHT;
			});
		}
		...
		bindTooltip() {
			...
			this.container.addEventListener('mousemove', function (e) {
				this$1.components.forEach(function (comp) {
					...
					if(daySquares.includes(daySquare)) {
						...
						var gOff = this$1.container.getBoundingClientRect(), pOff = daySquare.getBoundingClientRect();

						var width = parseInt(e.target.getAttribute('width'));
						var x = pOff.left - gOff.left + width/2;
						var y = pOff.top - gOff.top;
						var value = count + ' ' + this$1.countLabel;
						var name = ' on ' + month + ' ' + dateParts[0] + ', ' + dateParts[2];

						this$1.tip.setValues(x, y, {name: name, value: value, valueFirst: 1}, []);
						this$1.tip.showTip();
					}
				});
			});
		...
		renderLegend() {
			...
			this.colors.slice(0, HEATMAP_DISTRIBUTION_SIZE).map(function (color, i) {
				var square = heatSquare('heatmap-legend-unit', x + (COL_WIDTH + 3) * i,
					y, HEATMAP_SQUARE_SIZE, color);
				this$1.legendArea.appendChild(square);
			});
		...
		getDomains() {
			var s = this.state;
			var ref = [s.start.getMonth(), s.start.getFullYear()];
			var startMonth = ref[0];
			var startYear = ref[1];
			var ref$1 = [s.end.getMonth(), s.end.getFullYear()];
			var endMonth = ref$1[0];
			var endYear = ref$1[1];
			...
			for(var i = 0; i < noOfMonths; i++) {
				...
				if(!areInSameMonth(startOfMonth, s.end)) {
					var ref$2 = [startOfMonth.getMonth(), startOfMonth.getFullYear()];
					var month = ref$2[0];
					var year = ref$2[1];
		...
		getDomainConfig(startDate, endDate) {
			...
			var ref = [startDate.getMonth(), startDate.getFullYear()];
			var month = ref[0];
			var year = ref[1];
	...
	function dataPrep(data, type) {
		...
		datasets.map(function (d){
			...
			if(!d.values) {
				...
				var vals = d.values;
				vals = vals.map(function (val) { return (!isNaN(val) ? val : 0); });
		...
		if(data.yRegions) {
			data.yRegions.map(function (d) {
				...
				if(d.end < d.start) {
					(assign = [d.end, d.start], d.start = assign[0], d.end = assign[1]);
				}
			});
		}
	...
	function getShortenedLabels(chartWidth, labels, isSeries) {
		if ( labels === void 0 ) labels=[];
		if ( isSeries === void 0 ) isSeries=true;
		...
		if(isSeries) {
			...
			var maxLabelLength = Math.max.apply(Math, labels.map(function (label) { return label.length; }));
	...
	class AxisChart extends BaseChart {
		...
		calcXPositions() {
			...
			s.xAxis = {
				labels: labels,
				positions: labels.map(function (d, i) { return floatTwo(s.xOffset + i * s.unitWidth); }
				)
			};
		}

		calcYAxisParameters(dataValues, withMinimum) {
			if ( withMinimum === void 0 ) withMinimum = 'false';
			...
			this.state.yAxis = {
				labels: yPts,
				positions: yPts.map(function (d) { return zeroLine - d * scaleMultiplier; }),
		...
		calcDatasetPoints() {
			var s = this.state;
			var scaleAll = function (values) { return values.map(function (val) { return scale(val, s.yAxis); }); };
		...
		getAllYValues() {
			...
			if(this.barOptions.stacked) {
				...
				this.data.datasets.map(function (d, i) {
					var values = this$1.data.datasets[i].values;
					d[key] = cumulative = cumulative.map(function (c, i) { return c + values[i]; });
				});
			}

			var allValueLists = this.data.datasets.map(function (d) { return d[key]; });
			if(this.data.yMarkers) {
				allValueLists.push(this.data.yMarkers.map(function (d) { return d.value; }));
			}
			if(this.data.yRegions) {
			...
			return (ref = []).concat.apply(ref, allValueLists);
		}
		...
		setupComponents() {
			...
			var barDatasets = this.state.datasets.filter(function (d) { return d.chartType === 'bar'; });
			var lineDatasets = this.state.datasets.filter(function (d) { return d.chartType === 'line'; });
			...
			var barsConfigs = barDatasets.map(function (d) {
				...
				return [
					...
					{
						index: index,
						color: this$1.colors[index],
						stacked: this$1.barOptions.stacked,

						// same for all datasets
						valuesOverPoints: this$1.config.valuesOverPoints,
						minHeight: this$1.height * MIN_BAR_PERCENT_HEIGHT,
					},
					function() {
						var s = this.state;
						...
						var xPositions = s.xAxis.positions.map(function (x) { return x - barsWidth/2; });
						if(!stacked) {
							xPositions = xPositions.map(function (p) { return p + barWidth * index; });
						}
						...
						if(stacked) {
							offsets = d.yPositions.map(function (y, j) { return y - d.cumulativeYPos[j]; });
						}
			...
			var lineConfigs = lineDatasets.map(function (d) {
				...
				return [
					'lineGraph' + '-' + d.index,
					{
						index: index,
						color: this$1.colors[index],
						svgDefs: this$1.svgDefs,
						heatline: this$1.lineOptions.heatline,
						regionFill: this$1.lineOptions.regionFill,
						spline: this$1.lineOptions.spline,
						hideDots: this$1.lineOptions.hideDots,
						hideLine: this$1.lineOptions.hideLine,
					...
					function() {
						var s = this.state;
						var d = s.datasets[index];
						var minLine = s.yAxis.positions[0] < s.yAxis.zeroLine
			...
			this.components = new Map(componentConfigs
				.filter(function (args) { return !optionals.includes(args[0]) || this$1.state[args[0]]; })
				.map(function (args) {
					var component = getComponent.apply(void 0, args);
					if(args[0].includes('lineGraph') || args[0].includes('barGraph')) {
						this$1.dataUnitComponents.push(component);
					}
					return [args[0], component];
				}));
		}
		...
		makeDataByIndex() {
			...
			titles.map(function (label, index) {
				var values = this$1.state.datasets.map(function (set, i) {
					var value = set.values[index];
					return {
						title: set.name,
						value: value,
						yPos: set.yPositions[index],
						color: this$1.colors[i],
						formatted: formatY ? formatY(value) : value,
					};
				});

				this$1.dataByIndex[index] = {
					label: label,
		...
		bindTooltip() {
			...
			this.container.addEventListener('mousemove', function (e) {
				var m = this$1.measures;
				var o = getOffset(this$1.container);
				var relX = e.pageX - o.left - getLeftOffset(m);
				var relY = e.pageY - o.top;

				if(relY < this$1.height + getTopOffset(m)
					&& relY >  getTopOffset(m)) {
					this$1.mapTooltipXPosition(relX);
				} else {
					this$1.tip.hideTip();
				}
			});
			...
			if(s.datasets.length > 1) {
				...
				s.datasets.map(function (d, i) {
					...
					var rect = legendBar(
						...
						d.name,
						this$1.config.truncateLegends);
					this$1.legendArea.appendChild(rect);
				});
			}
		}
		...
		makeOverlay() {
			...
			this.overlayGuides.map(function (d) {
				var currentUnit = d.units[this$1.state.currentIndex];

				d.overlay = makeOverlay[d.type](currentUnit);
				this$1.drawArea.appendChild(d.overlay);
			});
		}
		...
		bindUnits() {
			...
			this.dataUnitComponents.map(function (c) {
				c.units.map(function (unit) {
					unit.addEventListener('click', function () {
						...
						this$1.setCurrentDataPoint(index);
					});
				});
			});
			...
			this.tip.container.addEventListener('click', function () {
				var index = this$1.tip.container.getAttribute('data-point-index');
				this$1.setCurrentDataPoint(index);
			});
		}
		...
		getDataPoint(index) {
			if ( index === void 0 ) index=this.state.currentIndex;

			var s = this.state;
			var data_point = {
				index: index,
				label: s.xAxis.labels[index],
				values: s.datasets.map(function (d) { return d.values[index]; })
			};
			return data_point;
		}
		...
		// API
		addDataPoint(label, datasetValues, index) {
			if ( index === void 0 ) index=this.state.datasetLength;
		...
		removeDataPoint(index) {
			if ( index === void 0 ) index = this.state.datasetLength-1;
	...
	class DonutChart extends AggregationChart {
		...
		calc() {
			...
			s.sliceTotals.map(function (total, i) {
				...
				var curPath =
					originDiffAngle === 360
						? makeStrokeCircleStr(curStart, curEnd, this$1.center, this$1.radius, this$1.clockWise, largeArc)
						: makeArcStrokePathStr(curStart, curEnd, this$1.center, this$1.radius, this$1.clockWise, largeArc);
	...
	function getChartByType(chartType, parent, options) {
...
//# sourceMappingURL=frappe_charts.min.js.map
